...
class TestGetSphinxRstPath:
    """Test Sphinx RST index.rst path resolution."""

    def test_returns_existing_file(self, env_sdk_path):
        result = _sdk_utils.get_sphinx_rst_path()
        assert result.exists()
        assert result.name == "index.rst"
...
class TestGetInitPyPath:
    """Test __init__.py path resolution."""

    def test_returns_existing_file(self, env_sdk_path):
        result = _sdk_utils.get_init_py_path()
        assert result.exists()
        assert result.name == "__init__.py"

    def test_path_under_sdk(self, env_sdk_path):
        result = _sdk_utils.get_init_py_path()
        expected = env_sdk_path / "ObjectWrapper" / "GlyphsApp" / "__init__.py"
        assert result == expected
